...
const step1Schema = z.object({
  ...
  trade_role: z.enum(['BUYER', 'SELLER', 'BOTH'], { required_error: 'Select a trade role' }),
  ...
  min_order_value: z.number({ invalid_type_error: 'Enter a valid amount', required_error: 'Required' }).min(1000, 'Minimum order must be at least ₹1,000'),
  max_order_value: z.number({ invalid_type_error: 'Enter a valid amount', required_error: 'Required' }),
...
export default function RegisterPage() {
  ...
  const { user, isLoading, setEnterprise } = useAuth();
  ...
  const submitRegistration = async () => {
    ...
    try {
      ...
      if (response.data.data.enterprise_id) {
         // Optionally fetch enterprise, but auth provider will fetch it naturally or we can mock wait
      }
...
function StepIndicator({ currentStep }: { currentStep: number }) {
  ...
  return (
    <div className="flex items-center justify-between mb-8 relative">
      <div className="absolute left-6 right-6 top-[15px] h-[2px] bg-border z-0" />
      {steps.map((step, idx) => {
...
// Step 3 Review
// ─────────────────────────────────────────────────────────────────────────────
import { formatCurrency as _formatCur } from '@/lib/utils';
        ...
        <div className="grid grid-cols-2 gap-y-4 gap-x-2">
          ...
          <div className="col-span-2 flex gap-4">
            <div>
              ...
              <p className="text-sm text-foreground">{_formatCur(enterprise.min_order_value)}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-wide text-muted-foreground mb-1">Max Order</p>
              <p className="text-sm text-foreground">{_formatCur(enterprise.max_order_value)}</p>
            </div>
          </div>
